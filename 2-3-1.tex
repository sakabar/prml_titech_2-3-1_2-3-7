...
\begin{frame}{平方完成}
 \begin{itemize}
  ...
  \item 式(\ref{003314_21Nov14})の左辺の形に直すことで、ガウス分布の指数部の形にする
        \begin{equation}
         -\frac{1}{2}(\bm{x}_a-\bm{\mu}_{a|b})^{T}\bm{\Sigma}_{a|b}^{-1}(\bm{x}_a-\bm{\mu}_{a|b}) = -\frac{1}{2}\bm{x}_a^{\mathrm{T}}\bm{\Sigma}_{a|b}^{-1}\bm{x}_a+\bm{x}_a^{\mathrm{T}}\bm{\Sigma}_{a|b}^{-1}\bm{\mu}_{a|b} + const\label{135015_17Nov14}
        \end{equation}
